...
const TrainerCourseDelete = () => {
  ...
  useEffect(() =>{
    userService?.chceckScheduleMatching(userName).then((response) => setResult(response.data));
  },[]);
  ...
  function handleRegister(e) {
    ...
    if (checkBtn.context._errors.length === 0) {
      ...
      userService.removeTrainerToNewCourse(userName, courseList).then(
        (response) => {
          setStateLoading(true);
...
export default TrainerCourseDelete;