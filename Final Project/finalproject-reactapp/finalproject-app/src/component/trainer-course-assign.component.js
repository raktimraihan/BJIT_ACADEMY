...
const TrainerCourseAssign = () => {
  ...
  useEffect(() =>{
    userService?.chceckSchedule(userName).then((response) => setResult(response.data));
  },[]);
  ...
  function handleRegister(e) {
    ...
    if (checkBtn.context._errors.length === 0) {
      ...
      userService.assignTrainerToNewCourse(userName, courseList).then(
        (response) => {
          setStateLoading(true);
...
export default TrainerCourseAssign;