...
import TableBodyQuizReviewed from "./quiz-reviewed-body.component";

const QuizViewResult = () => {
  const [quizInfo, setQuizInfo] = useState();

  useEffect(() => {
    userService
      ?.getTraineeResult(authService.getCurrentUser().userName)
      ...
      <table class="table col-md-12 table-striped table-dark table-bordered">
        ...
        <tbody>
          {quizInfo?.map((post, index) => (
            <TableBodyQuizReviewed index={index} post={post} />
          ))}
        </tbody>
      </table>
